...
12. **运行标题过滤**: 自动过滤页眉页脚的章节标题（如 "6    1. 不可压缩欧拉方程"）
13. **KaTeX兼容处理**: 自动转换 `\eqref` 为 `(\ref)` 确保前端渲染正常
...
3. **强大的LLM转换**: 支持 DeepSeek、GLM、GPT、Doubao 等主流大模型
...
12. **运行标题过滤**: 智能过滤页眉页脚中的章节标题
13. **KaTeX兼容处理**: 自动转换不支持的命令确保前端正常渲染
